...
class Sakura {
	...
	draw(cxt: CanvasRenderingContext2D) {
		...
		// Use transform instead of direct drawImage to support GPU acceleration
	...
	update() {
		...
		// If sakura goes out of bounds or becomes fully transparent, reset position
		if (
			...
			// If sakura has no limit
			if (this.limitArray[this.idx] === -1) {
				this.resetPosition();
			}
			// Otherwise sakura has limits
			else {
...
// Get sakura effect running status
