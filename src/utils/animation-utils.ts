/**
 * Animation utility class - Reference to yukina theme animation system
...
export class AnimationManager {
	...
	private setupSwupIntegration(): void {
		if (typeof window !== "undefined" && (window as any).swup) {
			...
			// Re-initialize animations after content replacement
	...
	private triggerPageLeaveAnimation(): void {
		...
		// Mobile optimization: reduce animation delay to avoid flickering
	...
	private initializePageAnimations(): void {
		...
		// Re-initialize sidebar components
	...
	private initializeSidebarComponents(): void {
		// Find sidebar element in page
		const sidebar = document.getElementById("sidebar");
		if (sidebar) {
			// Trigger custom event to notify sidebar re-initialization
			const event = new CustomEvent("sidebar:init");
			sidebar.dispatchEvent(event);
		}

		// Trigger global event to notify all components to re-initialize
