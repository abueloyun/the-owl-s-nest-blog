class CodeBlockCollapser {
	...
	setupThemeOptimizerSync() {
		// Sync with theme optimizer to ensure consistent hide/show behavior for code blocks
		...
		// Listen for page view events to ensure sync
		document.addEventListener("swup:pageView", () => {
			// Delay sync to ensure theme optimizer has processed code blocks
			setTimeout(() => {
				this.syncWithThemeOptimizer();
			}, 150);
		});
	...
	syncWithThemeOptimizer() {
		...
		if (window.themeOptimizer) {
			...
			// If theme optimizer not available, apply default behavior
			const codeBlocks = document.querySelectorAll(".expressive-code");
			codeBlocks.forEach((block) => {
				block.classList.add("hide-during-transition");
			});
	...
	setupThemeChangeListener() {
		// Listen for theme changes, pause observer during transition and optimize performance
			...
			for (const mutation of mutations) {
				if (
					...
					if (isTransitioning && !this.isThemeChanging) {
						...
						// Disconnect observer to avoid unnecessary checks during theme transition
						if (this.observer) {
							this.observer.disconnect();
						}

						// Performance optimization: temporarily disable animations and transitions for code blocks
						document
						...
						// Wait for theme transition to fully complete before restoring
						requestAnimationFrame(() => {
							// Restore transition effects for code blocks
							document
								.querySelectorAll(".expressive-code")
								.forEach((block) => {
	...
	createToggleButton() {
		...
		button.type = "button";
		button.setAttribute("aria-label", "Collapse/expand code block");
	...
	observePageChanges() {
		// If theme is changing, don't reconnect
		...
		this.observer = new MutationObserver((mutations) => {
			// If theme is changing, ignore all mutations
			if (this.isThemeChanging) {
				return;
			}
			...
			// Outer loop: iterate through all mutations
			for (const mutation of mutations) {
				if (
					mutation.type === "childList" &&
					mutation.addedNodes.length > 0
				) {
					// Inner loop: iterate through added nodes
...
function setupSwupHooks() {
	if (window.swup) {
		...
		window.swup.hooks.on("page:view", () => {
			...
			// Reset processedBlocks after page transition to ensure new page code blocks are processed
		...
		window.swup.hooks.on("content:replace", () => {
			...
			// Also reset on content replacement to ensure processing isn't skipped due to caching
			codeBlockCollapser.processedBlocks = new WeakSet();
			setTimeout(() => {
				codeBlockCollapser.setupCodeBlocks();
			}, 50);
...
// Try to setup Swup hooks immediately
if (!setupSwupHooks()) {
	// If Swup not yet initialized, wait for it to load
